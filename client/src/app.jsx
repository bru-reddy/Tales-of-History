...
import"./styles.css";
import topicFallback from"./staticTopics.js";
...
async function getTopics(){try{const d=await api("/topics");return Array.isArray(d)&&d.length>=80?d:topicFallback}catch{return topicFallback}}
async function getTopic(slug){try{return await api("/topics/"+slug)}catch{return topicFallback.find(x=>x.slug===slug)}}
...
"amaterasu-cave":commons("Amaterasu.png"),
"momotaro":"https://commons.wikimedia.org/wiki/Special:FilePath/Momotaro.jpg?width=1200",
"pangu-nuwa":commons("Chen Rong - Nine Dragons (dragons only).jpg"),
"chang-e":commons("Chang E, The Moon Goddess.jpg"),
"monkey-king":commons("Sun Wukong.jpg"),
...
const accurateTopicImages={
 ...
 "persian-empire":"https://commons.wikimedia.org/wiki/Special:FilePath/Achaemenid%20Empire%20under%20Cyrus.png",
 "ancient-greece":commons("The Parthenon in Athens.jpg"),
 "alexander":commons("Alexander the Great mosaic.jpg"),
 "roman-republic":"https://commons.wikimedia.org/wiki/Special:FilePath/Roman%20forum.jpg",
 "roman-empire":commons("Colosseum in Rome, Italy - April 2008.jpg"),
 "qin-han-china":"https://commons.wikimedia.org/wiki/Special:FilePath/Army%20of%20Terracotta.jpg",
 ...
 "crusades":"https://commons.wikimedia.org/wiki/Special:FilePath/Crusades.jpg",
 ...
 "macedonian-heracles":commons("Vergina Sun WIPO.svg"),
 "macedonian-dionysus":commons("Vergina Sun WIPO.svg"),
 "macedonian-orpheus":commons("Vergina Sun WIPO.svg"),
 "macedonian-vergina-sun":commons("Vergina Sun - Golden Larnax.png"),
 "macedonian-olympian-traditions":commons("Vergina Sun WIPO.svg"),
...
function ExplorerPage({kind,slug:forcedSlug}){
 ...
 useEffect(()=>{setLoading(true);getTopics().then(d=>{setTopics(d);setLoading(false)}).catch(()=>{setTopics(topicFallback);setLoading(false)})},[slug]);
...
function Topic(){
 const{slug}=useParams(),[t,setT]=useState(),[q,setQ]=useState(""),[a,setA]=useState(""),[notes,setNotes]=useState([]),[heroImage,setHeroImage]=useState("");
 useEffect(()=>{getTopic(slug).then(setT)},[slug]);
 useEffect(()=>{if(!t)return;const src=imageFor(t);const probe=new Image();probe.onload=()=>setHeroImage(src);probe.onerror=async()=>setHeroImage(await findRelatedImage(t));probe.src=src},[t]);
